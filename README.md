...
本仓库 fork 自 [duzyn/scoop-cn](https://github.com/duzyn/scoop-cn) 。这个版本的唯一改动是更新了运行间隔时间，使得更新更及时。其他部分与原仓库保持一致。
...
```powershell
irm https://ghproxy.cc/https://raw.githubusercontent.com/CZAsTc/scoop-cn/main/install.ps1 | iex
```

或使用 jsDelivr 的地址：

```powershell
irm https://cdn.jsdelivr.net/gh/CZAsTc/scoop-cn/install.ps1 | iex
```
...
1. 运行以下命令添加本仓库
    ...
    ```powershell
    scoop bucket add scoop-cn https://ghproxy.cc/https://github.com/CZAsTc/scoop-cn
    ```
...
git -C "$env:USERPROFILE\scoop\buckets\scoop-cn" remote set-url origin https://ghproxy.cc/https://github.com/CZAsTc/scoop-cn
```
...
![Star History Chart](https://api.star-history.com/svg?repos=CZAsTc/scoop-cn&type=Date)
